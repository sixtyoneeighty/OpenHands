...
import agentmojo
from agentmojo.controller.state.state import State
from agentmojo.core.config import AgentConfig, AppConfig, LLMConfig, SandboxConfig
from agentmojo.core.logger import agentmojo_logger as logger
from agentmojo.core.main import create_runtime, run_controller
from agentmojo.events.action import CmdRunAction, MessageAction
...
from agentmojo.events.stream import EventStreamSubscriber
from agentmojo.resolver.interfaces.github import GithubIssueHandler, GithubPRHandler
from agentmojo.resolver.interfaces.gitlab import GitlabIssueHandler, GitlabPRHandler
from agentmojo.resolver.interfaces.issue import Issue
from agentmojo.resolver.interfaces.issue_definitions import (
    ServiceContextIssue,
    ServiceContextPR,
)
from agentmojo.resolver.resolver_output import ResolverOutput
from agentmojo.resolver.utils import (
    Platform,
...
async def resolve_issue(
    ...
    if repo_instruction is None:
        # Check for .agentmojo_instructions file in the workspace directory
        agentmojo_instructions_path = os.path.join(repo_dir, '.agentmojo_instructions')
        if os.path.exists(agentmojo_instructions_path):
            with open(agentmojo_instructions_path, 'r') as f:
...
def main() -> None:
    ...
    if runtime_container_image is None and not my_args.is_experimental:
        runtime_container_image = (
            f'ghcr.io/all-hands-ai/runtime:{agentmojo.__version__}-nikolaik'
        )

...
from agentmojo.core.config import LLMConfig
from agentmojo.core.logger import agentmojo_logger as logger
from agentmojo.llm.llm import LLM
from agentmojo.resolver.interfaces.github import GithubIssueHandler
from agentmojo.resolver.interfaces.gitlab import GitlabIssueHandler
from agentmojo.resolver.interfaces.issue import Issue
from agentmojo.resolver.interfaces.issue_definitions import ServiceContextIssue
from agentmojo.resolver.io_utils import (
    load_all_resolver_outputs,
    load_single_resolver_output,
)
from agentmojo.resolver.patching import apply_diff, parse_patch
...
def make_commit(repo_dir: str, issue: Issue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        subprocess.run(
            f'git -C {repo_dir} config user.name "agentmojo" && '
            f'git -C {repo_dir} config user.email "agentmojo@all-hands.dev" && '
            f'git -C {repo_dir} config alias.git "git --no-pager"',
            shell=True,
            check=True,
        )
        print('Git user configured as agentmojo')
...
def send_pull_request(
    ...
    base_branch_name = f'agentmojo-fix-issue-{issue.number}'
    ...
    if additional_message:
        pr_body += f'\n\n{additional_message}'
    pr_body += '\n\nAutomatic fix generated by [AgentMojo](https://github.com/All-Hands-AI/AgentMojo/) 🙌'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'AgentMojo made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = f'A new AgentMojo update is available, but failed to parse or summarize the changes:\n{additional_message}'

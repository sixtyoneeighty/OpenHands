...
from agentmojo.core.logger import RollingLogger
from agentmojo.core.logger import agentmojo_logger as logger
from agentmojo.runtime.builder.base import RuntimeBuilder
from agentmojo.utils.term_color import TermColor, colorize
...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def build(
        ...
        if not DockerRuntimeBuilder.check_buildx(self.is_podman):
            # when running agentmojo in a container, there might not be a "docker"
            # binary available, in which case we need to download docker binary.
            # since the official agentmojo app image is built from debian, we use
            # debian way to install docker binary
            logger.info(
                'No docker binary available inside agentmojo-app container, trying to download online...'
            )
            commands = [
                'apt-get update',

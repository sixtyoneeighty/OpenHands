...
from agentmojo.events.action import Action, IPythonRunCellAction
from agentmojo.events.observation import IPythonRunCellObservation
from agentmojo.runtime.plugins.jupyter.execute_server import JupyterKernel
...
class JupyterPlugin(Plugin):
    ...
    async def initialize(self, username: str, kernel_id: str = 'agentmojo-default'):
        self.kernel_gateway_port = find_available_tcp_port(40000, 49999)
        self.kernel_id = kernel_id
        if username in ['root', 'agentmojo']:
            # Non-LocalRuntime
            prefix = f'su - {username} -s '
            # cd to code repo, setup all env vars and run micromamba
            poetry_prefix = (
                'cd /agentmojo/code\n'
                'export POETRY_VIRTUALENVS_PATH=/agentmojo/poetry;\n'
                'export PYTHONPATH=/agentmojo/code:$PYTHONPATH;\n'
                'export MAMBA_ROOT_PREFIX=/agentmojo/micromamba;\n'
                '/agentmojo/micromamba/bin/micromamba run -n agentmojo '
            )
        else:
            # LocalRuntime
            prefix = ''

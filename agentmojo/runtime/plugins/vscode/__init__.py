...
from agentmojo.runtime.utils.system import check_port_available
...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str):
        if username not in ['root', 'agentmojo']:
            self.vscode_port = None
            self.vscode_connection_token = None
            logger.warning(
                'VSCodePlugin is only supported for root or agentmojo user. '
        ...
        cmd = (
            ...
            f'sudo chown -R {username}:{username} /agentmojo/.openvscode-server\n'
            'cd /workspace\n'
            f'exec /agentmojo/.openvscode-server/bin/openvscode-server --host 0.0.0.0 --connection-token {self.vscode_connection_token} --port {self.vscode_port}\n'
            'EOF'
        )
        print(cmd)

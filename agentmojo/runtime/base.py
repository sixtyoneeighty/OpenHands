...
from agentmojo.core.config import AppConfig, SandboxConfig
from agentmojo.core.exceptions import AgentRuntimeDisconnectedError
from agentmojo.core.logger import agentmojo_logger as logger
from agentmojo.events import EventSource, EventStream, EventStreamSubscriber
...
from agentmojo.events.serialization.action import ACTION_TYPE_TO_CLASS
from agentmojo.integrations.github.github_service import GithubServiceImpl
from agentmojo.microagent import (
    BaseMicroAgent,
    load_microagents_from_dir,
)
from agentmojo.runtime.plugins import (
    JupyterRequirement,
    PluginRequirement,
    VSCodeRequirement,
)
from agentmojo.runtime.utils.edit import FileEditRuntimeMixin
...
class Runtime(FileEditRuntimeMixin):
    ...
    def clone_repo(
        ...
        agentmojo_workspace_branch = f'agentmojo-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {agentmojo_workspace_branch}'
        )
    ...
    def get_microagents_from_selected_repo(
        ...
        microagents_dir = workspace_root / '.agentmojo' / 'microagents'
        repo_root = None
        if selected_repository:
            repo_root = workspace_root / selected_repository.split('/')[-1]
            microagents_dir = repo_root / '.agentmojo' / 'microagents'
        self.log(
            'info',
        ...
        # Check for legacy .agentmojo_instructions file
        obs = self.read(
            FileReadAction(path=str(workspace_root / '.agentmojo_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.agentmojo_instructions not present, trying to load from repository {microagents_dir=}',
            )
            obs = self.read(
                FileReadAction(path=str(repo_root / '.agentmojo_instructions'))
            )

        if isinstance(obs, FileReadObservation):
            self.log('info', 'agentmojo_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroAgent.load(
                    path='.agentmojo_instructions', file_content=obs.content
                )
            )

...
It is responsible for executing actions received from AgentMojo backend and producing observations.
...
from agentmojo_aci.editor.editor import OHEditor
from agentmojo_aci.editor.exceptions import ToolError
from agentmojo_aci.editor.results import ToolResult
from agentmojo_aci.utils.diff import get_diff
from pydantic import BaseModel
from starlette.background import BackgroundTask
from starlette.exceptions import HTTPException as StarletteHTTPException
from uvicorn import run

from agentmojo.core.exceptions import BrowserUnavailableException
from agentmojo.core.logger import agentmojo_logger as logger
from agentmojo.events.action import (
    Action,
...
from agentmojo.events.event import FileEditSource, FileReadSource
from agentmojo.events.observation import (
...
from agentmojo.events.serialization import event_from_dict, event_to_dict
from agentmojo.runtime.browser import browse
from agentmojo.runtime.browser.browser_env import BrowserEnv
from agentmojo.runtime.plugins import ALL_PLUGINS, JupyterPlugin, Plugin, VSCodePlugin
from agentmojo.runtime.utils.bash import BashSession
from agentmojo.runtime.utils.files import insert_lines, read_lines
from agentmojo.runtime.utils.memory_monitor import MemoryMonitor
from agentmojo.runtime.utils.runtime_init import init_user_and_working_directory
from agentmojo.runtime.utils.system_stats import get_system_stats
from agentmojo.utils.async_utils import call_sync_from_async, wait_all
...
class ActionExecutor:
    ...
    async def ainit(self):
        ...
        if 'agent_skills' in self.plugins and 'jupyter' in self.plugins:
            obs = await self.run_ipython(
                IPythonRunCellAction(
                    code='from agentmojo.runtime.plugins.agent_skills.agentskills import *\n'
                )
            )
    ...
    async def _init_bash_commands(self):
        INIT_COMMANDS = [
            'git config --file ./.git_config user.name "agentmojo" && git config --file ./.git_config user.email "agentmojo@all-hands.dev" && alias git="git --no-pager" && export GIT_CONFIG=$(pwd)/.git_config'
            if os.environ.get('LOCAL_RUNTIME_MODE') == '1'
            else 'git config --global user.name "agentmojo" && git config --global user.email "agentmojo@all-hands.dev" && alias git="git --no-pager"'
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--username', type=str, help='User to run as', default='agentmojo'
    )

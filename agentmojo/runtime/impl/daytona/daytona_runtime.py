...
from agentmojo.runtime.plugins.requirement import PluginRequirement
from agentmojo.runtime.utils.command import get_action_execution_server_startup_command
from agentmojo.runtime.utils.request import RequestHTTPError
...
WORKSPACE_PREFIX = 'agentmojo-sandbox-'
...
class DaytonaRuntime(ActionExecutionClient):
    ...
    def _start_action_execution_server(self) -> None:
        ...
        start_command: list[str] = get_action_execution_server_startup_command(
            ...
            override_username='agentmojo',
        )
        start_command_str: str = (
            f'mkdir -p {self.config.workspace_mount_path_in_sandbox} && cd /agentmojo/code && '
            + ' '.join(start_command)
        )

...
import agentmojo
from agentmojo import __version__ as oh_version
from agentmojo.core.exceptions import AgentRuntimeBuildError
from agentmojo.core.logger import agentmojo_logger as logger
from agentmojo.runtime.builder import DockerRuntimeBuilder, RuntimeBuilder
...
def build_runtime_image(
    ...
    If dry_run is False, it will also build the AgentMojo runtime Docker image using the docker build folder.
...
def prep_build_folder(
    ...
    agentmojo_source_dir = Path(agentmojo.__file__).parent
    project_root = agentmojo_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'agentmojo' directory (Source code)
    shutil.copytree(
        agentmojo_source_dir,
        Path(build_folder, 'code', 'agentmojo'),
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(agentmojo_source_dir, file)
        if not src.exists():
...
def get_hash_for_lock_files(base_image: str):
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(agentmojo_source_dir, file)
        if not src.exists():
            src = Path(agentmojo_source_dir.parent, file)

...
Given the impressive capabilities of AgentMojo and similar coding agents, ensuring robust security measures is essential to prevent unintended actions or security breaches. The SecurityAnalyzer framework provides a structured approach to monitor and analyze agent actions for potential security risks.
...
For customization and decoupling from the AgentMojo core logic, the security analyzer can define its own API endpoints that can then be accessed from the frontend. These API endpoints need to be secured (do not allow more capabilities than the core logic
provides).

## How to implement your own Security Analyzer

1. Create a submodule in [security](/agentmojo/security/) with your analyzer's desired name
    * Have your main class inherit from [SecurityAnalyzer](/agentmojo/security/analyzer.py)
    * Optional: define API endpoints for `/api/security/{path:path}` to manage settings,
2. Add your analyzer class to the [options](/agentmojo/security/options.py) to have it be visible from the frontend combobox
...
It uses the [Invariant Analyzer](https://github.com/invariantlabs-ai/invariant) to analyze traces and detect potential issues with AgentMojo's workflow. It uses confirmation mode to ask for user confirmation on potentially risky actions.
...
* Logs:
    ...
    * AgentMojo traces in JSON format
* Run-time settings:

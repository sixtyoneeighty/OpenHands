...
from agentmojo.events.event import Event, EventSource, RecallType
from agentmojo.events.observation.agent import (
    MicroagentKnowledge,
    RecallObservation,
)
from agentmojo.events.observation.empty import NullObservation
from agentmojo.events.stream import EventStream, EventStreamSubscriber
...
from agentmojo.utils.prompt import RepositoryInfo, RuntimeInfo

GLOBAL_MICROAGENTS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(agentmojo.__file__)),
    'microagents',
)
...
class Memory:
    ...
    def __init__(
        ...
        # from typically AgentMojo/microagents (i.e., the PUBLIC microagents)

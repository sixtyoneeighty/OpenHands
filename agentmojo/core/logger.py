...
def get_file_handler(
    ...
    file_name = f'agentmojo_{timestamp}.log'
...
agentmojo_logger = logging.getLogger('agentmojo')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
agentmojo_logger.setLevel(current_log_level)

if DEBUG:
    agentmojo_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    agentmojo_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    agentmojo_logger.addHandler(json_log_handler(current_log_level))
else:
    agentmojo_logger.addHandler(get_console_handler(current_log_level))

agentmojo_logger.addFilter(SensitiveDataFilter(agentmojo_logger.name))
agentmojo_logger.propagate = False
agentmojo_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of agentmojo/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    agentmojo_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    agentmojo_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        agentmojo_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class AgentMojoLoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(self, logger=agentmojo_logger, extra=None):
        self.logger = logger
        self.extra = extra or {}

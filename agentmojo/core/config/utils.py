...
from agentmojo.core import logger
from agentmojo.core.config.agent_config import AgentConfig
from agentmojo.core.config.app_config import AppConfig
from agentmojo.core.config.condenser_config import condenser_config_from_toml_section
...
def load_from_toml(cfg: AppConfig, toml_file: str = 'config.toml') -> None:
    ...
    try:
        ...
        logger.agentmojo_logger.warning(
    ...
    # set LLMSummarizingCondenserConfig as default
        ...
        from agentmojo.core.config.condenser_config import LLMSummarizingCondenserConfig
    ...
    for key in toml_config:
        if key.lower() not in known_sections:
            logger.agentmojo_logger.warning(f'Unknown section [{key}] in {toml_file}')
...
def get_agent_config_arg(
    ...
    logger.agentmojo_logger.debug(f'Loading agent config from {agent_config_arg}')
    ...
    try:
        ...
        logger.agentmojo_logger.error(f'Config file not found: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.agentmojo_logger.error(
    ...
    logger.agentmojo_logger.debug(f'Loading from toml failed for {agent_config_arg}')
    return None
...
def get_llm_config_arg(
    ...
    logger.agentmojo_logger.debug(f'Loading llm config from {llm_config_arg}')
    ...
    logger.agentmojo_logger.debug(f'Loading from toml failed for {llm_config_arg}')
    return None
...
def parse_arguments() -> argparse.Namespace:
    ...
    if args.version:
        print(f'AgentMojo version: {__version__}')
        sys.exit(0)
